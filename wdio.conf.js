...
export const config = {
    ...
    before: [
      function () {
        ...
        const environmentProperties = {
          ...
          application_url: 'https://in10s.com',
          tested_username: 'testing@in10s.com',
        };
  
        const filePath = './allure-results/environment.properties';
  
        // Write the environment properties to the file
        fs.writeFileSync(
          filePath,
          Object.entries(environmentProperties)
            .map(([key, value]) => `${key}=${value}`)
            .join('\n')
        );
  
        // Log to the console for verification
        console.log(`Environment properties saved to ${filePath}`);
      },
      function () {
        const { spawn } = require('child_process');
  
        // Start the webdriver-manager in the background
    ...
    exclude: [
      ...
      // If you want to stop the webdriver-manager after tests, you can use the 'after' hook
      after: function () {
        //const { exec } = require('child_process');
        
        // Stop the webdriver-manager
        exec('webdriver-manager shutdown');
      },
